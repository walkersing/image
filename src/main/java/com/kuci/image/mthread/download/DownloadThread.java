package com.kuci.image.mthread.download;
	...
	public void run() {
		...
		try {
			...
			while ((read = in.read(bytes)) != -1) {
				...
				//					// 字节，则用要本线程总下载长度减去已经下载的长度
				//					read = length - count;
				//					System.out.println("最后一次的长度：" + read);
				//				}
				//				// 将准确的字节写入输出流
				...
				//				}
				//-----------------------------------------------------------------------------------------------				
				//就不知道上下两种写法有什么不同，有兴趣可以研究一下
				count += read;
                if (count > length) {
                	read = read - (count - length);
                }
                out.write(bytes, 0, read);
                //System.out.println("线程" + whichThread + "下载了" + count + "字节");
                if (count > length) {
                    break;
                }
			}
			//System.out.println("线程字节数：" + count  +  "计算的分配字节数："+length  +whichThread);
			// 将监视变量设置为true
			isFinish = true;
		} catch (Exception e) {
			e.printStackTrace();
